...
const VideoCall: React.FC<VideoCallProps> = ({ channelName, appId, token, uid, onLeave }) => {
  ...
  useEffect(() => {
    if (startedRef.current) {
      // Prevent StrictMode double-invoke from running setup twice
      return;
    }
    ...
    const joinChannel = async () => {
      try {
        ...
        // Prevent duplicate join when already connecting/connected
        // @ts-ignore - connectionState is available at runtime
        const state: string | undefined = (client as any).connectionState;
          ...
          if (!publishedRef.current) {
            try {
              ...
              if (msg.includes('CAN_NOT_PUBLISH_MULTIPLE_VIDEO_TRACKS')) {
                // Ignore: already has a published video track (likely from dev StrictMode)
              } else {
                throw err;
              }
            }
    ...
    return () => {
      ...
      try {
        ...
        if (publishedRef.current && localTracks.length > 0) {
          client.unpublish(localTracks as any).catch(() => {});
        }
      } catch {}
      try {
    ...
    // Do not depend on localTracks to avoid re-running and double publish
